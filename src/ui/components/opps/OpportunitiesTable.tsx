...
export default function OpportunitiesTable({ opportunities }: Props) {
  if (opportunities.length === 0) {
    return (
      <div className="mt-8">
        <div className="relative bg-white border border-gray-200 rounded-2xl p-8 shadow-lg transition-all duration-500">
    ...
    <div className="p-6">
            ...
            <tbody className="divide-y divide-gray-200/50">
              {opportunities.map((opp, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium border ${getStageColor(
                        opp.stage
                      )}`}
                    >
      ...
      <div className="md:hidden space-y-4 mb-6">
        {opportunities.map((opp, index) => (
          <div
            key={opp.id}
            className="relative bg-white border border-gray-200 rounded-xl p-4 shadow-lg hover:shadow-xl transition-all duration-300"
            style={{ animationDelay: `${index * 100}ms` }}
          >
